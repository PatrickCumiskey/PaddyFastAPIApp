...
def test_query_average(client: TestClient):
    ...
    # Check that we received a MultiSensorQueryResult with just sensor_ids
    assert "sensor_ids" in temperature_result
    assert "sensor_id" not in temperature_result
...
def test_query_min_max(client: TestClient):
    ...
    # Check that we received a SingleSensorQueryResult with just a sensor_id field
    assert "sensor_id" in max_result
    assert "sensor_ids" not in max_result
